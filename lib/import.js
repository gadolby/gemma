...
let importVCF = async function(filename, cmd) {
...
let importGFF = async function(filename, cmd) {
    let db = await jdb.Database(cmd.database);

    const empty = (await db.isEmpty('genes') && await db.isEmpty('subgenes'));
    if (empty) {
        return new Promise(function(resolve, reject) {
